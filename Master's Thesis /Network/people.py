...
dataloader = DataLoader(dataset, batch_size= 504, shuffle=True, num_workers=0)
model = GlassesNet()
model.to(device)
...
for walk in test:
    ...
    plt.yticks([0, 1, 2, 3, 4, 5, 6, 7], ["walk_male#1", "walk_male#2", "walk_female#1", "walk_female#2",
     "walk_male#3", "walk_female#3", "walk_male#4", "walk_female#4"])
    plt.figure()
    plt.plot(positions)
    plt.show()
