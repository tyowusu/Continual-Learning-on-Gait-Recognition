...
def decode_data():
    ...
    print("rh", "\t", "temp", "\t", "uva", "\t", "uvb", "\t", "x","\t", "y", "\t", "blueg", "\t", "blueb", "\t",
"worn", "\t", "pressure")

    for rh, temp, uva, uvb, x, y, blueg, blueb, worn, zero, pressure  in iter_unpack(data_format_env, data_env):
        print(rh/100.0, "\t", temp/100.0, "\t", uva*6.35/100.0, "\t", uvb*8.52/1000.0, "\t", x,"\t", y, "\t", blueg,
"\t", blueb, "\t", worn, "\t", pressure)

    print("accx", "\t", "accy","\t", "accz", "\t", "gyrx","\t", "gyry", "\t", "gyrz", "\t", "magx", "\t", "magy", "\t",
"magz", "\t", "roll", "\t", "pitch", "\t", "yaw")

    for accx, accy, accz, gyrx, gyry, gyrz, magx, magy, magz, roll, pitch, yaw in iter_unpack(data_format_imu,
data_imu):
        print(accx, "\t", accy,"\t", accz, "\t", gyrx/10,"\t", gyry/10, "\t", gyrz/10, "\t", magx/10, "\t", magy/10,
"\t", magz/10, "\t", roll/10, "\t", pitch/10, "\t", yaw/10)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mac", type=str, help="MAC address of the device", default="C8D5BEDC-A31E-A7E3-8931-183ED06EA614")#"D0:28:FE:C3:6C:D2")#"D2:60:7D:C0:B9:95")
    parser.add_argument("--duration", type=int, help="duration in seconds of the acquisition", default=10)
