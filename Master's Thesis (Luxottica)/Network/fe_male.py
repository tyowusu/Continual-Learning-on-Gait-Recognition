...
y = np.concatenate([
    ...
    np.zeros((len(data["walk_male#2"]), 1)),
    np.zeros((len(data["walk_male#3"]), 1)),
    np.zeros((len(data["walk_male#4"]), 1)),
    np.ones((len(data["walk_female#1"]), 1)),
    np.ones((len(data["walk_female#2"]), 1)),
    np.ones((len(data["walk_female#3"]), 1)),
    np.ones((len(data["walk_female#4"]), 1))
...
dataloader = DataLoader(dataset, batch_size=7168, shuffle=True, num_workers= 0)
model = GlassesNet()
model.to(device)
...
for walk in test:
    ...
    for label in torch.argmax(prediction, axis=1).reshape(-1):
        c_test.append(colors[label.item()])
    plt.scatter(np.arange(len(prediction)), torch.argmax(prediction, axis=1), c = c_test)
    plt.yticks([0, 1], ["walk_male", "walk_female"])
    plt.show()
    prediction = torch.nn.functional.softmax(prediction, dim=1).T.reshape(1, 2, -1)
    prediction = torch.nn.ReflectionPad1d(2)(prediction)
    prediction = torch.nn.functional.conv1d(prediction, torch.ones((2, 1, 5))/5, padding="valid", groups=2).detach().cpu().numpy()[0].T
    c_test = []
    position = 0
    positions = []
